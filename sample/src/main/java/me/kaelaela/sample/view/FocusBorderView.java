...
import me.kaelaela.sample.utils.DensityUtil;
...
public class FocusBorderView extends ImageView {
    ...
    public FocusBorderView(Context context, AttributeSet attrs) {
        ...
        this.setBackgroundResource(R.drawable.older_select);
...
/*        ViewGroup root = (ViewGroup) this.getParent();
        ...
        Rect fromRect = findLocationWithView(this);  //获取焦点框图片的位置
        Rect toRect = findLocationWithView(toView);  //获取焦点控件的位置
        
        int x = toRect.left - fromRect.left;
        int y = toRect.top - fromRect.top;        //横纵坐标移动距离

        int deltaX = (toView.getWidth() - this.getWidth()) / 2;

        int deltaY = (toView.getHeight() - this.getHeight()) / 2;
        x = DensityUtil.dip2px(this.getContext(), x + deltaX); //dp转px
    ...
    public Rect findLocationWithView(View view) {
        ...
        root.offsetDescendantRectToMyCoords(view, rect);  //获取控件形坐标
